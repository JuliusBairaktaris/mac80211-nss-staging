...
/* Idempotent: several paths release the peer buffer, and ath11k_nss_peer_delete
 * relies on the NULL to reject a second delete.
 */
static void ath11k_nss_peer_buf_put(struct ath11k_base *ab,
				    struct ath11k_peer *peer)
{
	if (!peer->nss.vaddr)
		return;
...
static void ath11k_nss_wifili_event_receive(struct ath11k_base *ab, struct nss_wifili_msg *msg)
{
	...
	case NSS_WIFILI_SEND_MESH_CAPABILITY_INFO:
		/* The reply's cap_info payload is not populated by every firmware
		 * line that answers this message, so capability is the response
		 * itself, not a field in it.
		 */
		if (response != NSS_CMN_RESPONSE_EMSG)
...
/* Both mesh table dumps carry a firmware-chosen entry count in front of a
 * variable-length array. Refuse a count the message cannot actually hold.
 */
static bool ath11k_nss_mesh_dump_entries_valid(struct ath11k_vif *arvif,
					       const struct nss_cmn_msg *cm,
					       size_t hdr_len, size_t entry_size,
					       u32 num_entries)
{
	/* cm->len excludes the common header, so it is exactly the payload the
	 * dump struct starts at - do not use nss_cmn_get_msg_len(), which adds
	 * the header back in and would over-count by its size.
	 */
	size_t avail = cm->len;

	if (avail >= hdr_len && num_entries <= (avail - hdr_len) / entry_size)
		return true;

	ath11k_warn(arvif->ar->ab,
		    "nss mesh dump claims %u entries, message payload is %zu bytes\n",
		    num_entries, avail);
	return false;
}
				     ...
				     struct nss_wifi_mesh_msg *msg)
{
	...
	/* nss_wifi_mesh_rx_msg_handler() deliberately exempts the two table-dump
	 * types from its length check, so num_entries is unvalidated firmware
	 * input: bound it by what the message actually carries before it sizes a
	 * copy out of the message buffer.
	 */
	if (!ath11k_nss_mesh_dump_entries_valid(arvif, &msg->cm,
						offsetof(struct nss_wifi_mesh_path_table_dump,
							 path_entry),
						sizeof(struct nss_wifi_mesh_path_dump_entry),
				    ...
				    struct nss_wifi_mesh_msg *msg)
{
	...
	if (!ath11k_nss_mesh_dump_entries_valid(arvif, &msg->cm,
						offsetof(struct nss_wifi_mesh_proxy_path_table_dump,
							 path_entry),
						sizeof(struct nss_wifi_mesh_proxy_path_dump_entry),
						mpp_dump->num_entries))
		return -EINVAL;
				...
				__attribute__((unused)) struct napi_struct *napi)
{
	 ...
	 * The metadata ends at metadata_len, so a headroom of exactly that holds
	 * it - and this is the mesh multicast receive path, so a drop here is a
	 * frame the bridge never sees.
	 */
	if (skb_headroom(skb) < metadata_len) {
		ath11k_warn(ab, "mesh link vap msdu from nss has invalid headroom %d\n",
			    skb_headroom(skb));
		dev_kfree_skb_any(skb);
		return;
	}
...
int ath11k_nss_vdev_create(struct ath11k_vif *arvif)
{
	...
	case NL80211_IFTYPE_MESH_POINT:
		...
		mod_timer(&arvif->nss.mpp_expiry_timer,
				jiffies + msecs_to_jiffies(2 * MSEC_PER_SEC));
		break;
#endif
	default:
		ret = -ENOTSUPP;
...
void ath11k_nss_ext_vdev_delete(struct ath11k_vif *arvif)
{
	...
	if (arvif->nss.ext_vdev_up)
		ath11k_nss_ext_vdev_down(arvif);
				 ...
				 struct ieee80211_sta *sta)
{
	...
	sinfo->rx_bytes = u64_stats_read(&stainfo->deflink.rx_stats.bytes) +
